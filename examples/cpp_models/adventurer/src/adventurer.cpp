...
string AdventurerState::text() const
{
  return "s" + to_string(state_id) + "-"
         + to_string(state_id / Adventurer::current_->size_);
}
  ...
  string map
      = num_goals == 2
            ...
            : (string("mapSize = 5 nGoal = 50\n")
               + string(
                     "0.5 0.5 0.5 0.5 0.5\n") // probabilities of being trapped
               + string("0.02 101 0.02 102 0.02 103 0.02 104 0.02 105 0.02 106 "
                        "0.02 107 0.02 108 0.02 109 0.02 110 0.02 111 0.02 112 "
                        "0.02 113 0.02 114 0.02 115 0.02 116 0.02 117 0.02 118 "
                        "0.02 119 0.02 120 0.02 121 0.02 122 0.02 123 0.02 124 "
                        "0.02 125 0.02 126 0.02 127 0.02 128 0.02 129 0.02 130 "
                        "0.02 131 0.02 132 0.02 133 0.02 134 0.02 135 0.02 136 "
                        "0.02 137 0.02 138 0.02 139 0.02 140 0.02 141 0.02 142 "
                        "0.02 143 0.02 144 0.02 145 0.02 146 0.02 147 0.02 148 "
                        "0.02 149 0.02 150\n") // goal probability, goal reward
               + string("0.3")                 // observation noise
               );
      ...
      else
      {
        ...
        if (trap_prob_[position] < 1.0)
          transition_probabilities_[s][a].push_back(
              State(goal_type * size_ + next_pos, 1 - trap_prob_[position]));
        // transition_probabilities_[s][a].push_back(State(next_pos, 1 -
        // trap_prob_[position]));
      }
    }
  }
...
double Adventurer::ObsProb(OBS_TYPE obs, const State& s, int a) const
{
  ...
  if (obs == goal_type)
    return 1 - obs_noise_;
  return (obs < num_goals_) ? (obs_noise_ / (num_goals_ - 1)) : 0.0;
}
...
void Adventurer::PrintTransitions() const
{
  ...
  for (int s = 0; s < NumStates(); s++)
  {
    ...
    for (int a = 0; a < NumActions(); a++)
    {
      cout << transition_probabilities_[s][a].size() << " outcomes for action "
           << a << endl;
...
Belief* Adventurer::InitialBelief(const State* start, string type) const
{
  ...
  {
    AdventurerState* particle = static_cast<AdventurerState*>(
        Allocate(goal * size_, goal_prob_[goal]));
...
ScenarioUpperBound* Adventurer::CreateScenarioUpperBound(
    string name, string particle_bound_name) const
{
  ...
  {
    return new LookaheadUpperBound(
        this, *this, CreateParticleUpperBound(particle_bound_name));
  }
  else
  {
      ...
      History& history) const
  {
    ...
    for (int i = 0; i < particles.size(); i++)
    {
      ...
      if (static_cast<AdventurerState*>(particle)->state_id
              % regdemo_model_->size_
          != regdemo_model_->size_ - 1)
      {
        at_goal = false;
      }
    }
...
ScenarioLowerBound* Adventurer::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
  ...
  {
    return new BlindPolicy(
        model, A_STAY, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "SMART")
  {
    return new AdventurerSmartPolicy(
        model, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "RANDOM")
  {
  ...
  else if (name == "MAJORITY")
  {
    ...
    const_cast<Adventurer*>(this)->ComputeDefaultActions("MDP");
    return new MajorityActionPolicy(
        model, *policy, CreateParticleLowerBound(particle_bound_name));
  }
  else
  {
...
void Adventurer::PrintObs(const State& state, OBS_TYPE obs, ostream& out) const
{
  out << obs << endl;
}
...
Belief* Adventurer::Tau(const Belief* belief, int action, OBS_TYPE obs) const
{
  ...
  for (int i = 0; i < particles.size(); i++)
  {
    ...
    const vector<State>& distribution
        = transition_probabilities_[GetIndex(state)][action];
...
void Adventurer::Observe(
    const Belief* belief, int action, map<OBS_TYPE, double>& obss) const
{
  const vector<State*>& particles
      = static_cast<const ParticleBelief*>(belief)->particles();
  for (int i = 0; i < particles.size(); i++)
...
public:
  AdventurerPOMCPPrior(const DSPOMDP* model)
    : POMCPPrior(model), reg_model_(static_cast<const Adventurer*>(model))
  {
  }
...
void Adventurer::PrintPOMDPX() const
{
  ...
  cout << "<?xml version='1.0' encoding='ISO-8859-1'?>" << endl
       << "<pomdpx version='1.0' id='tagxmlfac' "
       ...
       << "description" << endl
       << "</Description>" << endl
       << "<Discount>0.95</Discount>" << endl;

  cout << "<Variable>" << endl
       << "<StateVar vnamePrev=\"state_0\" vnameCurr=\"state_1\" "
          "fullyObs=\"false\"> <ValueEnum>"
       << endl;
  for (int s = 0; s < NumStates(); s++)
    cout << " s" << s;
  cout << " st </ValueEnum> </StateVar>" << endl
       << endl
  ...
  cout << " ot" << endl
       << "</ValueEnum>" << endl
       << "</ObsVar>" << endl
       << "<ActionVar vname=\"action\"> <ValueEnum> stay left right "
          "</ValueEnum> </ActionVar>"
       << endl
       << "<RewardVar vname=\"reward\"/> </Variable>" << endl;

  cout << "<InitialStateBelief> <CondProb> <Var> state_0 </Var> <Parent> null "
          "</Parent>"
       << endl
       << "<Parameter type = \"TBL\">" << endl;
  for (int goal = 0; goal < num_goals_; goal++)
  {
  ...
  cout << "<StateTransitionFunction> <CondProb> <Var> state_1 </Var> "
          "<Parent>action state_0</Parent>"
       << endl
       << "<Parameter type = \"TBL\">" << endl;
  for (int s = 0; s < NumStates(); s++)
  {
    for (int a = 0; a < NumActions(); a++)
    {
      ...
      for (int i = 0; i < transition_probabilities_[s][a].size(); i++)
      {
        ...
        cout << "<Entry> <Instance> " << actions[a] << " s" << s << " s"
             << next.state_id << " </Instance> <ProbTable> " << next.weight
             << " </ProbTable> </Entry>" << endl;
        sum += next.weight;
      }
  ...
  cout << "<Entry> <Instance> * st st </Instance> <ProbTable> 1.0 </ProbTable> "
          "</Entry>"
       << endl;
  cout << "</Parameter> </CondProb> </StateTransitionFunction>" << endl;

  cout << "<ObsFunction> <CondProb> <Var> obs </Var> <Parent>action "
          "state_1</Parent>"
       << endl
       << "<Parameter type = \"TBL\">" << endl;
  for (int a = 0; a < NumActions(); a++)
  {
    for (int s = 0; s < NumStates(); s++)
    {
      ...
      {
        cout << "<Entry> <Instance> " << actions[a] << " s" << s << " o" << o
             << " </Instance> <ProbTable> " << ObsProb(o, *(states_[s]), a)
             << " </ProbTable> </Entry>" << endl;
      }
    }
  }
  cout << "<Entry> <Instance> * st ot </Instance> <ProbTable> 1.0 </ProbTable> "
          "</Entry>"
       << endl;
  cout << "</Parameter> </CondProb> </ObsFunction>" << endl;

  cout << "<RewardFunction> <Func> <Var> reward </Var> <Parent>action "
          "state_0</Parent>"
       << endl
       << "<Parameter type = \"TBL\">" << endl;
  for (int a = 0; a < NumActions(); a++)
  ...
  cout << "<Entry> <Instance> * st </Instance> <ValueTable> 0 </ValueTable> "
          "</Entry>"
       << endl;
