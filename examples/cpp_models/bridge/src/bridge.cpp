...
BridgeState::BridgeState() : position(0)
{
}

BridgeState::BridgeState(int _position) : position(_position)
{
}
...
ScenarioLowerBound* Bridge::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
  ...
  else if (name == "CALL_FOR_HELP" || name == "DEFAULT")
  {
    return new BlindPolicy(
        this, HELP, CreateParticleLowerBound(particle_bound_name));
  }
  else
  {
...
{
public:
  BridgePOMCPPrior(const DSPOMDP* model) : POMCPPrior(model)
  {
  }
