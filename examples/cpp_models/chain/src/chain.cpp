...
ChainState::ChainState() : mdp_state(0)
{
}
...
{
  int num_mdp_states = mdp_transitions_.size(),
      num_mdp_actions = mdp_transitions_[0].size();
...
FullChainBelief::FullChainBelief(
    const DSPOMDP* model, int num_mdp_states, int num_mdp_actions, double alpha)
  : Belief(model), cur_state_(0)
{
...
vector<State*> FullChainBelief::Sample(int num_particles) const
{
  ...
  {
    ChainState* particle
        = static_cast<ChainState*>(model_->Allocate(-1, 1.0 / num_particles));
    ...
    for (int state = 0; state < num_mdp_states; state++)
    {
      for (int action = 0; action < num_mdp_actions; action++)
      {
        particle->SetTransition(
            state, action, Dirichlet::Next(alpha_[state][action]));
      }
    }
...
{
  return new FullChainBelief(
      model_, alpha_.size(), alpha_[0].size(), alpha_[0][0][0]);
}
...
SemiChainBelief::SemiChainBelief(
    const DSPOMDP* model, int num_mdp_states, int num_mdp_actions)
  : Belief(model), cur_state_(0)
{
...
void SemiChainBelief::Update(int action, OBS_TYPE obs)
{
  ...
  int status = ((action == Chain::ACTION_B && obs == 0)
                || (action == Chain::ACTION_A && obs != 0))
                   ? SUCCESS
                   : SLIP;
...
{
  int num_mdp_states = Chain::NUM_MDP_STATES,
      num_mdp_actions = model_->NumActions();
    ...
    OBS_TYPE& obs) const
{
  ...
  int next = Random::GetCategory(
      state.GetTransition(state.mdp_state, action), random_num);
...
Belief* Chain::InitialBelief(const State* start, string type) const
{
  ...
  {
    belief = new FullChainBelief(this, NUM_MDP_STATES, NumActions(), alpha_);
  }
  else if (type == "SEMI")
  {
...
ScenarioUpperBound* Chain::CreateScenarioUpperBound(
...
public:
  MeanMDPScenarioLowerBound(const Chain* model, Belief* belief = NULL)
    : ScenarioLowerBound(model, belief), chain_model_(model)
  {
  }
  ...
  {
    int num_mdp_states = chain_model_->NUM_MDP_STATES,
        num_mdp_actions = chain_model_->NumActions();
    ...
    for (int i = 0; i < particles.size(); i++)
    {
      ...
      for (int state1 = 0; state1 < num_mdp_states; state1++)
      {
        for (int action = 0; action < num_mdp_actions; action++)
        {
          for (int state2 = 0; state2 < num_mdp_states; state2++)
          {
            double prob1 = mean.GetTransition(state1, action, state2);
            double prob2 = state->GetTransition(state1, action, state2);
            mean.SetTransition(state1, action, state2, prob1 + prob2);
          }
        }
      }
    }
    ...
    for (int state1 = 0; state1 < num_mdp_states; state1++)
    {
      for (int action = 0; action < num_mdp_actions; action++)
      {
        for (int state2 = 0; state2 < num_mdp_states; state2++)
        {
          ...
          mean.SetTransition(state1, action, state2, prob / particles.size());
        }
      }
    }
    ...
    {
      State* particle = particles[i];
      ChainState* copy = static_cast<ChainState*>(chain_model_->Copy(particle));
...
public:
  MeanMDPPolicy(
      const Chain* model, ParticleLowerBound* bound, Belief* belief = NULL)
    : Policy(model, bound, belief), chain_model_(model)
  {
  }
      ...
      History& history) const
  {
    ...
    ChainState* mean
        = static_cast<ChainState*>(chain_model_->Allocate(-1, 1.0));
    mean->Init(num_mdp_states, num_mdp_actions);
    ...
    int action = mean->policy[static_cast<ChainState*>(particles[0])->mdp_state]
                     .action;
...
ScenarioLowerBound* Chain::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
  if (name == "TRIVIAL")
  {
  ...
  else if (name == "ACTION_A")
  {
    return new BlindPolicy(
        this, ACTION_A, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "ACTION_B")
  {
    return new BlindPolicy(
        this, ACTION_B, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "MEAN" || name == "DEFAULT")
  {
    // scenario_lower_bound_ = new MeanMDPScenarioLowerBound(this);
    return new MeanMDPPolicy(
        this, CreateParticleLowerBound(particle_bound_name));
  }
  else
  {
