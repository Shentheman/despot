...
{
  string map = string("mapSize = 3 4\n") + string("0 0 0 0\n")
               + string(
                     "0.9477809139837822 0.6205091684435805 0.3148059529873232 "
                     "0.05405447818338871\n")
               + string("-1 0 0 0\n");
...
void Navigation::Init(istream& is)
{
  ...
  for (int y = 0; y < ysize_; y++)
  {
    for (int x = 0; x < xsize_; x++)
    {
      ...
      for (int i = 0; i < flag_size_; i++)
        states_[flag_size_ * pos + i]
            = new NavigationState(flag_size_ * pos + i);
  ...
  for (int i = 0; i < flag_size_; i++)
  {
    states_[flag_size_ * trap_pos_ + i]
        = new NavigationState(flag_size_ * trap_pos_ + i);
  }
  ...
  for (int s = 0; s < states_.size(); s++)
  {
    ...
    for (int a = 0; a < NumActions(); a++)
    {
      ...
      { // Transition for remaining steps
...
ScenarioUpperBound* Navigation::CreateScenarioUpperBound(
...
ScenarioLowerBound* Navigation::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
...
void Navigation::PrintObs(const State& state, OBS_TYPE obs, ostream& out) const
{
  out << obs << endl;
}
