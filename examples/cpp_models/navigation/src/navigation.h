...
{
protected:
  static int flag_size_; // first 3 bits in state index: first_step_done,
                         // second_step_done, min-x
  ...
  std::vector<std::vector<std::vector<State> > >
      transition_probabilities_; // state, action, [state, weight]
...
public:
  ...
  virtual Belief* InitialBelief(
      const State* start, std::string type = "DEFAULT") const;

  inline double GetMaxReward() const
  {
    return 0;
  }
  ParticleUpperBound* CreateParticleUpperBound(
      std::string name = "DEFAULT") const;
  ScenarioUpperBound* CreateScenarioUpperBound(
  ...
  ScenarioLowerBound* CreateScenarioLowerBound(
      std::string name = "DEFAULT",
      std::string particle_bound_name = "DEFAULT") const;
  ...
  virtual void PrintObs(
      const State& state, OBS_TYPE obs, std::ostream& out = std::cout) const;
