    ...
    OBS_TYPE& obs) const
{
  ...
  reward = (state.state_id == size_ - 1 && action == A_STAY)
               ? goal_reward_
               : (action == A_STAY ? 0 : -1);
...
ScenarioUpperBound* RegDemo::CreateScenarioUpperBound(
...
ScenarioLowerBound* RegDemo::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
...
{
  return (s == size_ - 1 && action == 0) ? goal_reward_
                                         : (action == 0 ? 0 : -1);
}
