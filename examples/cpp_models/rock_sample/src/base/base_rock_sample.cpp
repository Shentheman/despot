...
BaseRockSample::BaseRockSample(int size, int rocks)
  : grid_(size, size), size_(size), num_rocks_(rocks)
{
  if (size == 4 && rocks == 4)
  {
    Init_4_4();
...
void BaseRockSample::Init_5_5()
{
  ...
  Coord rocks[]
      = {Coord(2, 4), Coord(0, 4), Coord(3, 3), Coord(2, 2), Coord(4, 1)};
...
public:
  RockSampleBelief(
      vector<State*> particles, const DSPOMDP* model, Belief* prior = NULL)
...
vector<State*> BaseRockSample::InitialParticleSet() const
{
  ...
  for (int n = 0; n < N; n++)
  {
    RockSampleState* rockstate
        = static_cast<RockSampleState*>(Allocate(n + pos, 1.0 / N));
...
vector<State*> BaseRockSample::NoisyInitialParticleSet() const
{
  ...
  for (int y = 1; y < size_ - 1; y++)
  {
    ...
    for (int n = 0; n < N; n++)
    {
      RockSampleState* rockstate = static_cast<RockSampleState*>(
          Allocate(n + pos, 1.0 / N / (size_ - 2)));
      particles.push_back(rockstate);
    }
  }
  return particles;
...
public:
  RockSampleParticleUpperBound1(const BaseRockSample* model) : rs_model_(model)
  {
  }
  ...
  double Value(const State& state) const
  {
    ...
    return 10.0 * (1 - Globals::Discount(count + 1))
           / (1 - Globals::Discount());
  }
};
...
public:
  RockSampleParticleUpperBound2(const BaseRockSample* model) : rs_model_(model)
  {
  }
      ...
      value += 10.0 * rs_model_->GetRock(&rockstate, rock)
               * Globals::Discount(
                     Coord::ManhattanDistance(
                         rs_model_->GetRobPos(&rockstate),
                         rs_model_->rock_pos_[rock]));
    value += 10.0 * Globals::Discount(
                        rs_model_->size_ - rs_model_->GetX(&rockstate));
    return value;
  }
};
...
public:
  ...
  double Value(const State& state) const
  {
    ...
    while (true)
    {
      ...
      {
        int dist
            = Coord::ManhattanDistance(rob_pos, rs_model_->rock_pos_[rock]);
      ...
      discount
          *= Globals::Discount(Coord::ManhattanDistance(rock_pos, rob_pos));
      value += discount * 10.0;
      visited[id] = true;
      rob_pos = rock_pos;
    }
...
ScenarioUpperBound* BaseRockSample::CreateScenarioUpperBound(
      ...
      History& history) const
  {
    return ValuedAction(
        ...
        10 * State::Weight(particles)
            * Globals::Discount(
                  grid_.xsize() - rs_model_->GetX(particles[0]) - 1));
  }
};
...
public:
    ...
    , grid_(rs_model_->grid_)
  {
    ...
    for (int s = 0; s < policy.size(); s++)
    {
      ...
      while (cur != policy.size() - 1)
      {
        ...
        {
          int rock
              = grid_(rs_model_->IndexToCoord(cur >> rs_model_->num_rocks_));
          if (rock < 0)
            exit(0);
  ...
  ValuedAction Value(
      ...
      History& history) const
  {
    vector<double> expected_sampling_value
        = vector<double>(rs_model_->num_rocks_);
    int state = 0;
    Coord rob_pos(-1, -1);
    ...
    for (int i = 0; i < particles.size(); i++)
    {
      ...
      state
          = (1 << rs_model_->num_rocks_) * rs_model_->GetRobPosIndex(particle);
      rob_pos = rs_model_->GetRobPos(particle);
      for (int r = 0; r < rs_model_->num_rocks_; r++)
      {
        expected_sampling_value[r]
            += particle->weight * (CheckFlag(particle->state_id, r) ? 10 : -10);
      }
...
public:
  RockSampleENTScenarioLowerBound(const DSPOMDP* model)
    : // ENT: Explore Nearest in Thresholded State
    ScenarioLowerBound(model)
    , rs_model_(static_cast<const BaseRockSample*>(model))
    , grid_(rs_model_->grid_)
  {
      ...
      History& history) const
  {
    ...
    for (int i = 0; i < particles.size(); i++)
    {
      ...
      for (int i = 0; i < rs_model_->num_rocks_; i++)
      {
        expected_sampling_value[i]
            += particle->weight * (rs_model_->GetRock(particle, i) ? 10 : -10);
      }
    }
    ...
    while (true)
    {
      ...
      /*
       ...
       Coord rock_pos(-1, -1);
       for (int rock=0; rock<rs_model_->num_rocks_; rock++) {
       double v = expected_sampling_value[rock] *
       Globals::Discount(Coord::ManhattanDistance(rob_pos,
       rs_model_->rock_pos_[rock]));
       if (v > best) {
       best = v;
       id = rock;
...
ScenarioLowerBound* BaseRockSample::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
  if (name == "TRIVIAL")
  {
...
public:
  ...
  ValuedAction Value(const Belief* belief) const
  {
    ...
    return ValuedAction(
        Compass::EAST,
        Globals::Discount(rs_model_->size_ - rs_model_->GetX(particles[0]) - 1)
            * 10);
  }
};
...
public:
  RockSampleMDPBeliefUpperBound(const BaseRockSample* model) : rs_model_(model)
  {
    policy_ = rs_model_->ComputeOptimalSamplingPolicy();
  }
...
public:
  RockSamplePOMCPPrior(const DSPOMDP* model)
    : POMCPPrior(model), rs_model_(static_cast<const BaseRockSample*>(model))
  {
  }
...
void BaseRockSample::PrintBelief(const Belief& belief, ostream& out) const
{
  ...
  for (int i = 0; i < particles.size(); i++)
  {
    ...
    for (int rock = 0; rock < num_rocks_; rock++)
      rock_probs[rock]
          += CheckFlag(particle->state_id, rock) * particle->weight;
...
Belief* BaseRockSample::Tau(
    const Belief* belief, int action, OBS_TYPE obs) const
{
    ...
    const Belief* belief, int action, map<OBS_TYPE, double>& obss) const
{
  ...
  { // Sense a rock
    for (int i = 0; i < particles.size(); i++)
    {
      ...
      double distance
          = Coord::EuclideanDistance(GetRobPos(&next), rock_pos_[rock]);
      double efficiency
          = (1 + pow(2, -distance / half_efficiency_distance_)) * 0.5;
...
bool BaseRockSample::GetObservation(
    double rand_num, const RockSampleState& rockstate, int rock) const
{
  double distance
      = Coord::EuclideanDistance(GetRobPos(&rockstate), rock_pos_[rock]);
