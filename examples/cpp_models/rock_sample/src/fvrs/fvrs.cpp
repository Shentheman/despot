...
FVRS::FVRS(string map) : BaseRockSample(map)
{
  half_efficiency_distance_ = (size_ - 1) * sqrt(2) / 4 / 2;
}

FVRS::FVRS(int size, int rocks) : BaseRockSample(size, rocks)
{
  half_efficiency_distance_ = (size_ - 1) * sqrt(2) / 4 / 2;
}
...
double FVRS::ObsProb(OBS_TYPE obs, const State& state, int action) const
{
  ...
  for (int rock = 0; rock < num_rocks_; rock++)
  {
    ...
    prob
        *= (((GetRock(&rockstate, rock) & 1) == bit) ? efficiency
                                                     : (1 - efficiency));
  }
  return prob;
}

int FVRS::GetObservation(
    double rand_num, const RockSampleState& rockstate) const
{
  int obs = 0;
  for (int rock = 0; rock < num_rocks_; rock++)
  {
