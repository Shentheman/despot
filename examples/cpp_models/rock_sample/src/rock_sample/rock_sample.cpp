...
RockSample::RockSample(string map) : BaseRockSample(map)
{
  half_efficiency_distance_ = 20;
}

RockSample::RockSample(int size, int rocks) : BaseRockSample(size, rocks)
{
  half_efficiency_distance_ = 20;
}

bool RockSample::Step(
    State& state,
    double rand_num,
    int action,
    double& reward,
...
double RockSample::ObsProb(OBS_TYPE obs, const State& state, int action) const
{
  ...
  return ((GetRock(&rockstate, rock) & 1) == obs) ? efficiency
                                                  : (1 - efficiency);
}

void RockSample::PrintObs(
