    ...
    OBS_TYPE& obs) const
{
  ...
  else if (rover_position == MIDDLE)
  {
    if (action == A_SAMPLE)
    {
      ...
      // moving does not incur observation, setting a default observation
      obs = O_GOOD;
      return true; // sampling in the grid where there is no rock terminates the
                   // task
    }
    else if (action == A_CHECK)
    {
      reward = 0;
      // when the rover is at MIDDLE, its observation is correct with
      // probability 0.8
...
double SimpleRockSample::ObsProb(
    OBS_TYPE obs, const State& state, int action) const
{
  if (action == A_CHECK)
  {
    ...
    {
      // when the rover at MIDDLE, its observation is correct with probability
      // 0.8
      return (obs == rock_status) ? 0.8 : 0.2;
    }
  }

  // when the actions are not A_CHECK, the rover does not receive any
  // observations.
...
Belief* SimpleRockSample::InitialBelief(const State* start, string type) const
{
  ...
  {
    cerr << "[SimpleRockSample::InitialBelief] Unsupported belief type: "
         << type << endl;
    exit(1);
  }
}
...
public:
  SimpleRockSampleParticleUpperBound(const DSPOMDP* model)
  {
    ...
    upper_bounds_[1].push_back(
        Globals::Discount(1) * 10 + Globals::Discount(3) * 10);
...
ScenarioUpperBound* SimpleRockSample::CreateScenarioUpperBound(
...
ScenarioLowerBound* SimpleRockSample::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
  ...
  {
    bound = new SimpleRockSampleEastPolicy(
...
void SimpleRockSample::PrintState(const State& state, ostream& out) const
{
  ...
  out << "Rover = " << simple_state.rover_position
      << "; Rock = " << (simple_state.rock_status ? "GOOD" : "BAD") << endl;
}

void SimpleRockSample::PrintObs(
    const State& state, OBS_TYPE observation, ostream& out) const
{
...
void SimpleRockSample::PrintBelief(const Belief& belief, ostream& out) const
{
  ...
  out << "The weighted averages among " << particles.size()
      << " particles:" << endl;
