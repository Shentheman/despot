...
TagBelief::TagBelief(
    vector<State*> particles, const BaseTag* model, Belief* prior)
  : ParticleBelief(particles, model, prior, false), tag_model_(model)
{
}
...
class TagSHRPolicy : public Policy
{ // Smart History-based Rollout
private:
...
public:
  TagSHRPolicy(const DSPOMDP* model, ParticleLowerBound* bound)
    : Policy(model, bound), tag_model_(static_cast<const BaseTag*>(model))
  {
...
public:
  ...
  {
    const vector<State*>& particles
        = static_cast<ParticleBelief*>(belief)->particles();
    double maxWeight = 0;
    State mode;
  ...
  void belief(Belief* belief)
  {
    ...
    else
    {
      ...
      first_action_
          = (policy.size() > 0) ? policy[0] : Random::RANDOM.NextInt(4);
    }
...
public:
  TagPOMCPPrior(const DSPOMDP* model)
    : POMCPPrior(model), tag_model_(static_cast<const BaseTag*>(model))
  {
  }
    ...
    if (history_.Size() != 0)
    {
      ...
      else
      {
        if (tag_model_->robot_pos_unknown_)
        {
          for (int a = 0; a < 4; a++)
          {
            if (tag_model_->floor_.Inside(rob + Compass::DIRECTIONS[a]))
            {
...
class TagManhattanUpperBound : public ParticleUpperBound,
                               public BeliefUpperBound
{
protected:
  const BaseTag* tag_model_;
...
public:
  TagManhattanUpperBound(const BaseTag* model) : tag_model_(model)
  {
...
class TagSPParticleUpperBound : public ParticleUpperBound
{ // Shortest path
protected:
  const BaseTag* tag_model_;
  vector<double> value_;
...
public:
  TagSPParticleUpperBound(const BaseTag* model) : tag_model_(model)
  {
    Floor floor = tag_model_->floor_;
    value_.resize(tag_model_->NumStates());
    for (int s = 0; s < tag_model_->NumStates(); s++)
...
public:
  TagBlindBeliefPolicy(const BaseTag* model, Belief* belief = NULL)
    : BeliefLowerBound(model, belief), tag_model_(model)
  {
  ...
  ValuedAction Value(const Belief* belief) const
  {
    ...
    for (int action = 0; action < tag_model_->NumActions(); action++)
    {
      double value = tag_model_->ComputeActionValue(
          static_cast<const ParticleBelief*>(belief), *tag_model_, action);
      if (value > bestValue)
      {
        bestValue = value;
        bestAction = action;
...
BaseTag::BaseTag() : robot_pos_unknown_(false)
{
  current_ = this;
  istringstream iss(
      string("mapSize = 5 10\n") + string("#####...##\n")
...
BaseTag::BaseTag(string params_file) : robot_pos_unknown_(false)
{
  current_ = this;
...
void BaseTag::Init(istream& is)
{
  ...
  for (int s = 0; s < NumStates(); s++)
  {
    ...
    for (int a = 0; a < NumActions(); a++)
    {
      ...
      if (!(a == TagAction() && rob_[s] == opp_[s]))
      { // No transition upon termination
        for (map<int, double>::const_iterator it = opp_distribution.begin();
             it != opp_distribution.end();
             it++)
        {
          State next;
...
bool BaseTag::Step(
    State& s, double random_num, int action, double& reward) const
{
  ...
  for (int s = 0; s < NumStates(); s++)
  {
    ...
    for (int a = 0; a < NumActions(); a++)
    {
      ...
      for (int i = 0; i < transition_probabilities_[s][a].size(); i++)
      {
        ...
        cout << s << "-" << a << "-" << next.state_id << "-" << next.weight
             << endl;
...
ScenarioUpperBound* BaseTag::CreateScenarioUpperBound(
    string name, string particle_bound_name) const
{
  if (name == "TRIVIAL" || name == "DEFAULT" || name == "MDP" || name == "SP"
      || name == "MANHATTAN")
  {
...
ScenarioLowerBound* BaseTag::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
  ...
  {
    return new TagSHRPolicy(
        model, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "HM")
  {
    return new TagHistoryModePolicy(
        model, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "MMAP-MDP")
  {
    ComputeDefaultActions("MDP");
    return new MMAPStatePolicy(
        model,
        *mmap_inferencer,
        *policy,
...
Coord BaseTag::MostLikelyOpponentPosition(const vector<State*>& particles) const
{
...
const TagState& BaseTag::MostLikelyState(const vector<State*>& particles) const
{
...
const State* BaseTag::GetMMAP(const vector<State*>& particles) const
{
  ...
  int state_id
      = RobOppIndicesToStateIndex(floor_.GetIndex(rob), floor_.GetIndex(opp));
