...
NoisyLaserTag::NoisyLaserTag() : BaseTag(), noise_sigma_(2.5), unit_size_(1.0)
{
...
NoisyLaserTag::NoisyLaserTag(string params_file)
  : BaseTag(params_file), noise_sigma_(2.5), unit_size_(1.0)
{
  Init();
  robot_pos_unknown_ = true;
}
...
{
  Coord rob = floor_.GetCell(rob_[state.state_id]),
        opp = floor_.GetCell(opp_[state.state_id]);
  int d = 1;
  while (true)
  {
    ...
    for (int d = 0; d < NBEAMS; d++)
    {
      ...
      for (int reading = 0; reading < dist / unit_size_; reading++)
      {
        ...
        double prob = 2 * (gausscdf(max_noise, 0, noise_sigma_)
                           - (reading > 0 ? gausscdf(min_noise, 0, noise_sigma_)
                                          : 0 /*min_noise = -infty*/));
...
bool NoisyLaserTag::Step(
    State& state, double random_num, int action, double& reward) const
{
    ...
    OBS_TYPE& obs) const
{
  ...
  else
  {
    ...
    {
      const vector<vector<double> >& distribution
          = reading_distributions_[state.state_id];
...
void NoisyLaserTag::PrintObs(
    const State& state, OBS_TYPE obs, ostream& out) const
{
  for (int i = 0; i < NBEAMS; i++)
...
Belief* NoisyLaserTag::InitialBelief(const State* start, string type) const
{
  ...
  for (int rob = 0; rob < N; rob++)
  {
    ...
    {
      TagState* state = static_cast<TagState*>(
          Allocate(RobOppIndicesToStateIndex(rob_[start->state_id], opp), wgt));
...
ostream& operator<<(ostream& os, const NoisyLaserTag& lasertag)
{
  ...
  {
    os << "State " << s << " " << lasertag.opp_[s] << " " << lasertag.rob_[s]
       << " " << lasertag.floor_.NumCells() << endl;
