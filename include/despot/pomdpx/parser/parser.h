...
private:
  ...
  std::vector<std::vector<bool> > is_identity_; // is_identity_[s][a] = true iff
                                                // transition_funcs_[s] is an
                                                // identity for action a

  // Convenience and sanity-check functions for POMDPX
  TiXmlElement* GetParameterElement(TiXmlElement* func_element);
  inline std::string GetFirstChildText(
      TiXmlElement* elem, const char* child) const;
  inline TiXmlElement* GetFirstChildElement(
      TiXmlElement* elem, const char* child) const;

  void Ensure(
      bool condition, std::string message, TiXmlBase* base = NULL) const;
  // void EnsureIdentityIsValid(TiXmlBase* base, std::vector<std::string>
  // tokens) const; // TODO
  int ComputeNumOfEntries(
      const std::vector<std::string>& instance,
      const std::vector<NamedVar*>& parents,
  ...
  ObsVar CreateObsVar(TiXmlElement* varChild);
  ActionVar CreateActionVar(TiXmlElement* varChild);
  RewardVar CreateRewardVar(TiXmlElement* varChild);
...
public:
  ...
  */
  ...
  double GetReward(
      const std::vector<int>& perv_state,
      const std::vector<int>& curr_state,
      int action) const;
  double GetReward(int action) const;
  void GetNextState(std::vector<int>& state, int action, double& random) const;
  void GetNoisyNextState(
      std::vector<int>& state, int action, double& random) const;
  OBS_TYPE GetObservation(
      const std::vector<int>& state, int action, double& random) const;
  inline bool IsTerminalState(const std::vector<int>& state) const
  {
    return has_terminal_ && (IsInTerminalStateSet(state)
                             || IsSelfLoopingWithoutReward(state));
  }
  ...
  void PrintState(
      const std::vector<int>& state, std::ostream& out = std::cout) const;
