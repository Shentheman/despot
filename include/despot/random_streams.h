...
/**
 * A RandomStreams object represents multiple random number sequences, where
 * each
...
private:
  std::vector<std::vector<double> >
      streams_; // streams_[i] is associated with i-th particle
...
public:
  ...
  friend std::ostream& operator<<(
      std::ostream& os, const RandomStreams& stream);
};
