    ...
    for (int s = 0; s < num_states; s++)
    {
      ...
      for (int a = 0; a < num_actions; a++)
      {
        ...
        for (int i = 0; i < transition.size(); i++)
        {
          ...
          v += next.weight * Globals::Discount() * policy_[next.state_id].value;
        }
...
void MDP::ComputeBlindAlpha()
{
  ...
  for (int action = 0; action < num_actions; action++)
  {
    ...
    logi << "[MDP::ComputeBlindAlpha] Tol(alpha_" << action << ") after "
         << iter << " iters = " << tol << endl;
...
double MDP::ComputeActionValue(
    const ParticleBelief* belief, const StateIndexer& indexer, int action) const
{
  ...
  for (int i = 0; i < particles.size(); i++)
  {
    ...
    value
        += particle->weight * blind_alpha_[action][indexer.GetIndex(particle)];
  }
