...
double ParticleUpperBound::Value(
...
double TrivialParticleUpperBound::Value(
    const vector<State*>& particles,
    RandomStreams& streams,
    History& history) const
{
  return State::Weight(particles) * model_->GetMaxReward()
         / (1 - Globals::Discount());
}
...
LookaheadUpperBound::LookaheadUpperBound(
  ...
  : model_(model), indexer_(indexer), particle_upper_bound_(bound)
{
}
  ...
  for (int p = 0; p < num_particles; p++)
  {
    ...
    for (int t = length; t >= 0; t--)
    {
      ...
      { // base case
        ...
        {
          bounds_[p][t][s]
              = particle_upper_bound_->Value(*indexer_.GetState(s));
        }
      }
      else
      { // lookahead
...
double LookaheadUpperBound::Value(
    const vector<State*>& particles,
    RandomStreams& streams,
    History& history) const
{
  ...
  {
    State* particle = particles[i];
    bound += particle->weight
             * bounds_[particle->scenario_id][streams.position()]
                      [indexer_.GetIndex(particle)];
  }
  return bound;
}
...
MDPUpperBound::MDPUpperBound(const MDP* model, const StateIndexer& indexer)
  : model_(model), indexer_(indexer)
{
