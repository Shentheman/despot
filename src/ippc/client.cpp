...
{
  int status;
  //	    struct addrinfo host_info;       // The struct that getaddrinfo()
  //fills up with data.
  //	    struct addrinfo *host_info_list; // Pointer to the to the linked list
  //of host_info's.

  // The MAN page of getaddrinfo() states "All  the other fields in the
  // structure pointed
  // to by hints must contain either 0 or a null pointer, as appropriate." When
  // a struct
  // is created in c++, it will be given a block of memory. This memory is not
  // nessesary
  // empty. Therefor we use the memset function to make sure all fields are
  // NULL.
  ...
  cout << "port: " << PORT << endl;
  status = getaddrinfo(
      HOSTNAME.c_str(), PORT.c_str(), &host_info, &host_info_list);
...
string Client::createActionMes(
    const string actionName, const string actionValue1)
{
  TiXmlDocument doc;
