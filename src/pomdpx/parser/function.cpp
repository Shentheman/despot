...
Function::Function(NamedVar* child, vector<NamedVar*> parents)
  : child_(child), parents_(parents)
{
  int parent_size = 1;
  for (int i = 0; i < parents.size(); i++)
    parent_size *= parents[i]->values().size();

  if (child->values().size() < 100)
  { // TODO: possible refactoring to use a single data structure
    ...
    int end)
{
  if (key_pos == parents_.size() + 1)
  {
    ...
    if (values_.size()
        > 0) // TODO: possible refactoring to use a single data structure
  ...
  {
    NamedVar* curr = (key_pos == keys.size() - 1) ? child_ : parents_[key_pos];
    ...
    if (keys[key_pos] == "*")
    {
      for (int i = 0; i < dom.size(); i++)
      {
        ...
        if (!SetValue(
                keys, key_pos + 1, next_pid, next_cid, values, start, end))
          return false;
      }
      return true;
    }
    ...
    else
    {
      ...
      if (id < 0)
      {
        cerr << "ERROR: " << keys[key_pos] << " is not in the domain of "
             << curr->name() << endl;
        return false;
      }
...
bool Function::SetValue(
    const vector<string>& keys, const vector<double>& values)
{
...
double Function::ComputeConstrainedMaximum(const NamedVar* var, int value) const
{
...
double Function::ComputeConstrainedMinimum(const NamedVar* var, int value) const
{
  if (var->values().size() <= value)
    throw std::invalid_argument(
        "Variable " + var->name() + " cannot have value " + to_string(value)
...
ostream& operator<<(ostream& os, const Function& func)
{
  ...
  os << func.child_->name() << "]" << endl; // TODO: should print curr_name here
                                            // but prev_name above for state
                                            // variable
    ...
    for (int p = 0; p < func.values_.size(); p++)
    {
      ...
      for (int c = 0; c < func.values_[p].size(); c++)
      {
        if (func.values_[p][c] != 0)
          os << " " << func.child_->GetValue(c) << ":" << func.values_[p][c];
      }
  ...
  else
  {
    for (int p = 0; p < func.map_.size(); p++)
    {
      ...
      for (map<int, double>::const_iterator it = func.map_[p].begin();
           ...
           it++)
      {
        os << " " << func.child_->GetValue(it->first) << ":" << it->second;
      }
...
void TabularCPT::ComputeSparseChildDistribution()
{
  ...
  for (int p = 0; p < parent_size; p++)
  {
    ...
    {
      if (GetValue(p, c) > 0)
        sparse_values_[p].push_back(pair<int, double>(c, GetValue(p, c)));
    }
  }
}

bool TabularCPT::IsIdentityUnderConstraint(const NamedVar* var, int value) const
{
  if (var->values().size() <= value)
    throw std::invalid_argument(
        "Variable " + var->name() + " cannot have value " + to_string(value)
...
CPT* TabularCPT::CreateNoisyVariant(double noise) const
{
  ...
  for (int p = 0; p < parent_size; p++)
  {
    for (int c = 0; c < child_size; c++)
    {
      cpt->values_[p][c] = (GetValue(p, c) + noise / child_size) / (1 + noise);
    }
  }
...
bool HierarchyCPT::SetValue(
    int val, const vector<string>& keys, const vector<double>& values)
{
...
bool HierarchyCPT::IsIdentityUnderConstraint(
    const NamedVar* var, int value) const
{
  if (var == parents_[0])
...
CPT* HierarchyCPT::CreateNoisyVariant(double noise) const
{
  ...
  for (int i = 0; i < cpts_.size(); i++)
    hcpt->cpts_[i]
        = static_cast<TabularCPT*>(cpts_[i]->CreateNoisyVariant(noise));
  return hcpt;
}
