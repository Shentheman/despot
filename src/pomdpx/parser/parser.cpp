...
Parser::Parser() : enable_selfloop_cache_(true)
{
}

Parser::Parser(string fn) : enable_selfloop_cache_(true)
{
  Parse(fn);
  // Check();
}
...
ValuedAction Parser::ComputeMaxRewardAction() const
{
  ...
  for (int a = 0; a < action_vars_[0].Size(); a++)
  {
    ...
    for (int r = 0; r < reward_funcs_.size(); r++)
    {
      reward += reward_funcs_[r].ComputeConstrainedMaximum(&action_vars_[0], a);
    }
    if (reward > max.value)
    {
...
ValuedAction Parser::ComputeMinRewardAction() const
{
  ...
  for (int a = 0; a < action_vars_[0].Size(); a++)
  {
    ...
    for (int r = 0; r < reward_funcs_.size(); r++)
    {
      reward += reward_funcs_[r].ComputeConstrainedMinimum(&action_vars_[0], a);
    }
    if (reward > min.value)
    {
...
void Parser::Parse(string fn)
{
  ...
  logi << "Done [" << double(clock() - start) / CLOCKS_PER_SEC << "s]" << endl;
  ...
  // ReadBounds();
  logi << "Parsing done in " << double(clock() - start) / CLOCKS_PER_SEC << "s"
       << endl;
}
...
void Parser::ParseDiscountTag(TiXmlHandle& xml_handle)
{
  TiXmlElement* e_Discount
      = xml_handle.FirstChild("pomdpx").FirstChild("Discount").ToElement();
  ...
  Ensure(
      Globals::config.discount < 1.0 && Globals::config.discount > 0,
...
{
  TiXmlElement* e_Horizon
      = xml_handle.FirstChild("pomdpx").FirstChild("Horizon").ToElement();
  if (e_Horizon != NULL)
  {
...
void Parser::ParseHasTerminalTag(TiXmlHandle& xml_handle)
{
  TiXmlElement* e_HasTerminal
      = xml_handle.FirstChild("pomdpx").FirstChild("HasTerminal").ToElement();
...
void Parser::ParseVariableTag(TiXmlHandle& xml_handle)
{
  TiXmlElement* e_Variable
      = xml_handle.FirstChild("pomdpx").FirstChild("Variable").ToElement();
  ...
  while (e_Var != 0)
  {
    ...
    {
      Ensure(action_vars_.size() == 0, "Only one action variable is allowed!");
...
void Parser::ParseTerminalStateTag(TiXmlHandle& handle)
{
  TiXmlElement* e_Terminal
      = handle.FirstChild("pomdpx").FirstChild("TerminalState").ToElement();
  ...
  while (e_Table != NULL)
  {
    ...
    while (e_Instance != NULL)
    {
      ...
      {
        values.push_back(
            curr_state_vars_[pattern.state_ids[i]].IndexOf(instance[i]));
      }
...
{
  TiXmlElement* e_CondProb = handle.FirstChild("pomdpx")
                                 .FirstChild("InitialStateBelief")
                                 .FirstChild("CondProb")
                                 .ToElement();
  ...
  while (e_CondProb != NULL)
  {
    ...
    Ensure(
        belief_function.Validate(),
        "In <InitialStateBelief>: Probability distribution does not sum to "
        "one!",
        e_CondProb);
...
TabularCPT Parser::CreateInitialBelief(TiXmlElement* e_CondProb)
{
  ...
  Ensure(
      Variable::IsVariableName(name, prev_state_vars_),
  ...
  vector<string> parent_names
      = Tokenize(GetFirstChildText(e_CondProb, "Parent"));
  // logi << "Parents: " << parent_names << endl;

  Ensure(
      (parent_names.size() == 1 && parent_names[0] == "null"),
      "In <InitialStateBelief>: Var " + name
  ...
  {
    // <Instance>
    vector<string> instance = Tokenize(GetFirstChildText(e_Entry, "Instance"));
    ...
    else
    {
      ...
      Ensure(
          value_tokens.size() == num_entries,
          "In <InitialStateBelief>: " + to_string(num_entries)
              + " entries expected but "
              + to_string(value_tokens.size())
              + " found.",
          GetFirstChildElement(e_Entry, "ProbTable"));
    ...
    Ensure(
        success,
        "In <InitialStateBelief>: Value assignment failed.",
...
{
  TiXmlElement* e_StateTransition = handle.FirstChild("pomdpx")
                                        .FirstChild("StateTransitionFunction")
                                        .ToElement();

  Ensure(e_StateTransition != NULL, "Cannot find StateTransitionFunction tag");
  ...
  while (e_CondProb != 0)
  {
    ...
    Ensure(
        transition->Validate(),
        "In <StateTransitionFunction>: Probability distribution does not sum "
        "to one!",
        e_CondProb);
  ...
  int num_states = transition_funcs_.size(),
      num_actions = action_vars_[0].Size();
  ...
  for (int s = 0; s < num_states; s++)
  {
    ...
    {
      is_identity_[s][a] = transition_funcs_[s]->IsIdentityUnderConstraint(
          &action_vars_[0], a);
    }
  }
}
...
void Parser::ParseObsFunctionTag(TiXmlHandle& handle)
{
  if (obs_vars_.size() > 0)
  {
    TiXmlElement* e_ObsFunction
        = handle.FirstChild("pomdpx").FirstChild("ObsFunction").ToElement();
...
TabularCPT* Parser::CreateTabularObsFunction(TiXmlElement* e_CondProb)
{
  ...
  while (e_Entry != NULL)
  {
    ...
    Ensure(
        success,
        "In <ObsFunction>: Value assignment failed.",
        GetFirstChildElement(e_Entry, "Instance"));
...
{
  TiXmlElement* e_RewardFunction
      = handle.FirstChild("pomdpx").FirstChild("RewardFunction").ToElement();
  ...
  Ensure(
      reward_funcs_.size() == reward_vars_.size(),
...
Function Parser::CreateRewardFunction(TiXmlElement* e_Func)
{
  ...
  Ensure(
      Variable::IsVariableName(name, reward_vars_),
      "In <RewardFunction>: Variable " + name
          + " has not been declared as a RewardVar vname within <Variable> "
            "tag.",
  ...
  while (e_Entry != NULL)
  {
    ...
    Ensure(
        ...
        "In <RewardFunction>: " + to_string(num_entries)
            ...
            + to_string(value_tokens.size())
            + " found.",
        GetFirstChildElement(e_Entry, "ValueTable"));
    ...
    Ensure(
        success,
        "In <RewardFunction>: Value assignment failed.",
        GetFirstChildElement(e_Entry, "Instance"));
...
int Parser::ComputeNumOfEntries(
    const vector<string>& instance,
    const vector<NamedVar*>& parents,
    const NamedVar* child) const
{
  int num_entries = 1;
  for (int i = 0; i < instance.size(); i++)
  {
...
string Parser::GetFirstChildText(TiXmlElement* elem, const char* child) const
{
  return elem->FirstChildElement(child)->GetText();
}

TiXmlElement* Parser::GetFirstChildElement(
    TiXmlElement* elem, const char* child) const
{
...
double Parser::GetReward(
    const vector<int>& prev_state,
    const vector<int>& curr_state,
    int action) const
{
  for (int s = 0; s < prev_state_vars_.size(); s++)
...
void Parser::GetNextState(vector<int>& state, int action, double& random) const
{
  for (int s = 0; s < state.size(); s++)
  {
    prev_state_vars_[s].curr_value = state[s];
...
void Parser::GetNoisyNextState(
    vector<int>& state, int action, double& random) const
{
  for (int s = 0; s < state.size(); s++)
  {
    prev_state_vars_[s].curr_value = state[s];
...
OBS_TYPE Parser::GetObservation(
    const vector<int>& state, int action, double& random) const
{
  for (int s = 0; s < state.size(); s++)
  {
    curr_state_vars_[s].curr_value = state[s];
...
double Parser::ObsProb(OBS_TYPE obs, const vector<int>& state, int action) const
{
  for (int s = 0; s < state.size(); s++)
  {
    curr_state_vars_[s].curr_value = state[s];
    ...
    const vector<int>& state, int action, int num) const
{
  ...
  for (int s = 0; s < state.size(); s++)
  {
    ...
    int pid = Variable::ComputeCurrentIndex(transition_funcs_[s]->parents());
    for (int v = 0; v < prev_state_vars_[s].Size(); v++)
    {
...
bool Parser::IsSelfLoopingWithoutReward(const vector<int>& state) const
{
  ...
  {
    if (selfloop_state_set_.size() > 10000)
    { // Replace the oldest state by the new one
