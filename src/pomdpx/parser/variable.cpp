...
vector<int> Variable::ComputeIndexVec(const vector<Variable*>& vars, int index)
{
...
vector<int> Variable::ComputeIndexVec(const vector<NamedVar*>& vars, int index)
{
  vector<int> vec(vars.size());
  for (int i = vec.size() - 1; i >= 0; i--)
  {
...
vector<int> Variable::ComputeIndexVec(const vector<StateVar*>& vars, int index)
{
  vector<int> vec(vars.size());
  for (int i = vec.size() - 1; i >= 0; i--)
  {
...
NamedVar::NamedVar(string name) : name_(name)
{
}
