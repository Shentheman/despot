    ...
    OBS_TYPE& obs) const
{
  ...
  reward = parser_->GetReward(
      action); // Prev state and curr state set in GetNextState
...
public:
  ...
  void Update(int action, OBS_TYPE obs)
  {
    ...
    for (int i = 0; i < max_iter_; i++)
    {
      for (int j = 0; j < particles_.size(); j++)
      {
        ...
        bool terminal = model_->Step(
            *copy, Random::RANDOM.NextDouble(), action, reward, o);
        double prob = model_->ObsProb(obs, *copy, action);

        if (!terminal && prob)
        { // Terminal state is not required to be explicitly represented and may
          // not have any observation
          copy->weight *= prob;
    ...
    if (updated.size() != particles_.size())
    {
      for (int i = 0; i < max_iter_ + 1; i++)
      {
        for (int j = 0; j < particles_.size(); j++)
        {
          ...
          bool terminal
              = model_->NoisyStep(*copy, Random::RANDOM.NextDouble(), action);
          double prob = model_->ObsProb(obs, *copy, action);

          if (i == max_iter_ && prob < 1E-6) // NOTE: never kill a particle at
                                             // the last iteration
            prob = 1E-6;
    ...
    if (num_effective_particles < num_particles_ / 20.0)
    { // NOTE: small particles may be removed...
      logi << "[POMDPXBelief::Update] Resampling " << num_particles_
           << " as effective number of particles = " << num_effective_particles
           << endl;
...
vector<State*> POMDPX::ExactInitialParticleSet() const
{
  ...
  for (int s = 0; s < NumStates(); s++)
  {
    POMDPXState* state = static_cast<POMDPXState*>(
        Allocate(s, parser_->InitialWeight(states_[s]->vec_id)));
    if (state->weight == 0)
    {
      Free(state);
    }
...
vector<State*> POMDPX::ApproxInitialParticleSet() const
{
  ...
  logi << "[POMDPX::ApproximatePrior] Drawing 1000 random particles from "
          "initial belief."
       << endl;
...
{
  vector<State*> particles;
  if (is_small_ && parser_->LogNumInitialStates() < 13)
  { // Small problem
    ...
    for (int a = 0; a < NumActions(); a++)
    {
      ...
      for (int i = 0; i < transition.size(); i++)
      {
        ...
        rewards_[s][a]
            += next.weight
               * parser_->GetReward(
                     states_[s]->vec_id, states_[next.state_id]->vec_id, a);
      }
    }
  }
}
...
public:
  POMDPXGreedyActionPolicy(const DSPOMDP* model, ParticleLowerBound* bound)
    : Policy(model, bound), pomdpx_model_(static_cast<const POMDPX*>(model))
  {
  }
      ...
      History& history) const
  {
    ...
    for (int action = 0; action < pomdpx_model_->NumActions(); action++)
    {
      ...
      for (int i = 0; i < particles.size(); i++)
      {
        ...
        reward += state->weight
                  * pomdpx_model_->parser_->GetReward(
                        state->vec_id, state->vec_id, action);
      }
      if (reward > maxReward)
      {
        maxReward = reward;
...
ScenarioLowerBound* POMDPX::CreateScenarioLowerBound(
    string name, string particle_bound_name) const
{
  ...
  else if (name == "MAJORITY")
  {
    ...
    return new MajorityActionPolicy(
        this, *policy, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "GREEDY")
  {
    return new POMDPXGreedyActionPolicy(
        this, CreateParticleLowerBound(particle_bound_name));
  }
  else if (name == "MODE")
  {
    ...
    return new ModeStatePolicy(
        this, *indexer, *policy, CreateParticleLowerBound(particle_bound_name));
  }
  else
  {
...
ScenarioUpperBound* POMDPX::CreateScenarioUpperBound(
    string name, string particle_bound_name) const
{
  if (!is_small_)
  {
  ...
  else if (name == "LOOKAHEAD")
  {
    return new LookaheadUpperBound(
        this, *indexer, CreateParticleUpperBound(particle_bound_name));
  }
  else
  {
