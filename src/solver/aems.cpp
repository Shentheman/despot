...
ValuedAction AEMS::Search()
{
  ...
  statistics_.num_tree_particles
      = model_->NumActiveParticles() - num_active_particles;
...
void AEMS::FindMaxApproxErrorLeaf(
    VNode* vnode, double likelihood, double& bestAE, VNode*& bestNode)
{
  if (vnode->IsLeaf())
  {
    double curAE = likelihood * vnode->likelihood
                   * Globals::Discount(vnode->depth())
...
void AEMS::InitLowerBound(
    VNode* vnode, BeliefLowerBound* lower_bound, History& history)
{
  double value = lower_bound->Value(vnode->belief()).value;
  vnode->lower_bound(value);
}

void AEMS::InitUpperBound(
    VNode* vnode, BeliefUpperBound* upper_bound, History& history)
{
    ...
    History& history)
{
  ...
  for (map<OBS_TYPE, double>::iterator it = obss.begin(); it != obss.end();
       it++)
  {
    OBS_TYPE obs = it->first;
    double weight = it->second;
    logd << "[AEMS::Expand] Creating node for obs " << obs << " with weight "
         << weight << endl;
    VNode* vnode = new VNode(
        model->Tau(belief, action, obs), parent->depth() + 1, qnode, obs);
