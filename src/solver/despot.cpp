    ...
    SearchStatistics* statistics)
{
  ...
  do
  {
    ...
    {
      ROS_ERROR_STREAM(
          "The QNode with the best upper_bound_ = " << *qstar << ", action="
                                                    << qstar->edge());
    }
    // TIGER:
    ...
    {
      ROS_ERROR_STREAM(
          "The vnode with max WEU = " << *next << ", observation="
                                      << next->edge());
      // TIGER:
    ...
    if (logging::level() >= logging::DEBUG)
    {
      cout << "After an iteration, the new cur=" << *cur << ", WEU=" << WEU(cur)
           << ", depth=" << cur->depth() << ", particles:" << endl;
      ...
      if (cur->depth() >= Globals::config.search_depth)
        ROS_ERROR_STREAM(
            "[DESPOT::Trial()] Exit the loop because "
            << "(cur->depth() >= Globals::config.search_depth)"
            << "\n==========================================\n");
      if (WEU(cur) <= 0)
        ROS_ERROR_STREAM(
            "[DESPOT::Trial()] Exit the loop because WEU(cur) <= 0"
            << "\n==========================================\n");
    }
    ...
    SearchStatistics* statistics)
{
  ...
  do
  {
    ...
    double start = clock();
    VNode* cur = Trial(
        root, streams, lower_bound, upper_bound, model, history, statistics);
    ...
    if (logging::level() >= logging::DEBUG)
    {
      if (used_time * (num_trials + 1.0) / num_trials >= timeout)
        ROS_ERROR_STREAM(
            "[DESPOT::ConstructTree()] Exit the loop because "
            << "time is up (timeout="
            << timeout
            << ")"
            << "\n==========================================\n");
...
void DESPOT::Compare()
{
  ...
  RandomStreams streams = RandomStreams(
      Globals::config.num_scenarios, Globals::config.search_depth);

  VNode* root = ConstructTree(
      particles,
      streams,
      lower_bound_,
      upper_bound_,
    ...
    History& history)
{
  ...
  if (logging::level() >= logging::DEBUG)
  {
    cout << "At depth=" << vnode->depth()
         << ", lower_bound (action, value) =" << move << endl;
    // TIGER:
    ...
    History& history)
{
  ...
  if (logging::level() >= logging::DEBUG)
  {
    cout << "At depth=" << vnode->depth() << ", upper_bound value =" << upper
         << endl;
    // TIGER:
...
ValuedAction DESPOT::Search()
{
  ...
  {
    return ValuedAction(
        Random::RANDOM.NextInt(model_->NumActions()), Globals::NEG_INFTY);
  }
  ...
  static RandomStreams streams = RandomStreams(
      Globals::config.num_scenarios, Globals::config.search_depth);
  ...
  if (logging::level() >= logging::DEBUG)
  {
    logi << "[DESPOT::Search] Time for deleting tree: "
         << (get_time_second() - start) << "s" << endl;
    logi << "[DESPOT::Search] Search statistics:" << endl
         << statistics_ << endl;
  }
...
ValuedAction DESPOT::OptimalAction(VNode* vnode)
{
  ...
  {
    ROS_ERROR_STREAM(
        "Default move=" << vnode->default_move() << " is better than astar="
                        << astar);
    exit(0);
    ...
    History& history)
{
  ...
  {
    ROS_ERROR_STREAM(
        "At depth=" << parent->depth() << ", Globals::Discount(depth)="
                    << Globals::Discount(parent->depth())
                    << ", Globals::config.pruning_constant="
                    << Globals::config.pruning_constant
                    << ", step_reward from all particles="
                    << step_reward);
       ...
       it++)
  {
    ...
    if (logging::level() >= logging::DEBUG)
    {
      cout << "partitions[" << counter << "]: obs=" << obs
           << ", action=" << qnode->edge() << ", " << it->second.size()
           << " particles=" << *vnode << endl;
    }
