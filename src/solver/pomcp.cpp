...
POMCPPrior::POMCPPrior(const DSPOMDP* model) : model_(model)
{
  exploration_constant_
      = (model->GetMaxReward() - model->GetMinRewardAction().value);
}
...
UniformPOMCPPrior::UniformPOMCPPrior(const DSPOMDP* model) : POMCPPrior(model)
{
}
...
POMCP::POMCP(const DSPOMDP* model, POMCPPrior* prior, Belief* belief)
  : Solver(model, belief), root_(NULL)
{
  reuse_ = false;
  prior_ = prior;
  assert(prior_ != NULL);
...
ValuedAction POMCP::Search(double timeout)
{
  ...
  logi << "[POMCP::Search] Search statistics" << endl
       ...
       << "Time: CPU / Real = " << ((clock() - start_cpu) / CLOCKS_PER_SEC)
       << " / " << (get_time_second() - start_real) << endl
...
int POMCP::UpperBoundAction(const VNode* vnode, double explore_constant)
{
  ...
  /*
   ...
   double ub = qnodes[action]->value() + explore_constant *
   sqrt(log(vnode->count() + 1) / qnodes[action]->count());
   cout << action << " " << ub << " " << qnodes[action]->value() << " " <<
   qnodes[action]->count() << " " << vnode->count() << endl;
   }
   */
...
ValuedAction POMCP::OptimalAction(const VNode* vnode)
{
  ...
  for (int action = 0; action < qnodes.size(); action++)
  {
    // cout << action << " " << qnodes[action]->value() << " " <<
    // qnodes[action]->count() << " " << vnode->count() << endl;
...
VNode* POMCP::CreateVNode(
    int depth, const State* state, POMCPPrior* prior, const DSPOMDP* model)
{
  ...
  { // no prior knowledge, all actions are equal
    ...
    POMCPPrior* prior)
{
  ...
  bool terminal = model->Step(
      *particle, streams.Entry(particle->scenario_id), action, reward, obs);
...
// static
double POMCP::Simulate(
    State* particle, VNode* vnode, const DSPOMDP* model, POMCPPrior* prior)
{
  assert(vnode != NULL);
...
// static
double POMCP::Rollout(
    State* particle, int depth, const DSPOMDP* model, POMCPPrior* prior)
{
    ...
    POMCPPrior* prior)
{
  ...
  for (int i = 0; i < particles.size(); i++)
  {
    ...
    {
      int action = (cur != NULL) ? UpperBoundAction(cur, 0)
                                 : prior->GetAction(*particle);

      double reward;
      OBS_TYPE obs;
      bool terminal = model->Step(
          *copy, streams.Entry(copy->scenario_id), action, reward, obs);
...
ValuedAction DPOMCP::Search(double timeout)
{
  ...
  RandomStreams streams(
      Globals::config.num_scenarios, Globals::config.search_depth);

  root_ = ConstructTree(particles, streams, model_, prior_, history_, timeout);
